#!/usr/bin/env python3
"""Fetch fixture results from FullTime API, scrape player events, update DB, run scoring.
...
4. Scrape player events from FullTime website (goals, assists, substitutions)
5. Generate PlayerGameweekPoints from scraped events
6. Score the gameweek (including transfer hits)
7. Update team standings and player season stats
8. If all fixtures played, prepare next gameweek

Data sources:
- Local FullTime API (localhost:5000) for team-level match results
- FullTime website (fulltime.thefa.com) for per-match player events
...
Usage:
    ...
    python scripts/fetch-and-score.py --score-only  # Skip API fetch, just score existing fixtures
...
def generate_player_gameweek_points(db: Session, gw_id: int) -> int:
    ...
    For normal fixtures, uses appearance points for all players.

    NOTE: This is a fallback. The preferred method is to scrape player events
    from the FullTime website using scrape_fulltime_stats.py --fixture-id <id>.
    ...
    processed = {(pid,) for pid in existing}
    if existing:
    ...
    for fixture in fixtures:
        ...
        if is_walkover:
            ...
            # Award 2 pts to winning team only (home team wins by default)
            ...
            for player in team_to_players.get(winner_team_id, []):
                ...
                pgp = PlayerGameweekPoints(
                    ...
                    was_home=(winner_team_id == fixture.home_team_id),
                    minutes_played=0,
                    did_play=True,
                    goals_scored=0,
                    clean_sheet=False,
        ...
        # Normal fixture - award appearance points to all players
        for team_id, goals_scored, goals_conceded, is_home in [
            ...
            for player in team_to_players.get(team_id, []):
                pid = player.id
                if (pid,) in processed:
                    continue
                processed.add((pid,))
                ...
                pgp = PlayerGameweekPoints(
                    ...
                    minutes_played=90,
                    did_play=True,
                    goals_scored=0,  # Use scrape_fulltime_stats.py for real goals
                    assists=0,  # Use scrape_fulltime_stats.py for real assists
                    clean_sheet=clean_sheet,
                    goals_conceded=goals_conceded,
                    saves=0,
                    base_points=MINUTES_60_PLUS,  # Appearance points only
                    total_points=MINUTES_60_PLUS,
                    bps_score=2,
                )
                db.add(pgp)
                created += 1
    ...
    print(f"  NOTE: Use scrape_fulltime_stats.py --fixture-id <id> to add real goals/assists")
    return created
...
def main():
    ...
    try:
        ...
        # 2. Fetch results from local API
        ...
        print("  TIP: Run scrape_fulltime_stats.py --fixture-id <id> to add real goals/assists")
